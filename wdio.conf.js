...
exports.config = {
  ...
  specs: ['./test/specs/**/*.js'],
  //  specs: ["./test/features/subtasks.feature"],
  // Patterns to exclude.
  exclude: [],
  //
  // ============
  ...
  capabilities: [
    {
      ...
      browserName: "chrome",
      chromeOptions: {
        args: ["window-size=1280x1024", "disable-gpu"]
      }
    }
  ],
  //
  ...
  before: function before() {
    ...
    mkdirp("./screenshots", function(err) {
      if (err) {
        console.log("Erro ao criar a pasta de screenshots", err);
      }
    });
  },
  //
    ...
    try {
      browser.saveScreenshot(
        "./screenshots/" +
          ...
          "_" +
          step.text.replace(/[\s\/]/g, "_") +
          "_before" +
          ".png"
      );
    } catch (err) {
      console.log("Erro ao salvar screenshot: " + err);
    }
  },
  //
